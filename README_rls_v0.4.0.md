...
- <ins>`-F`</ins>：ソート順を変更
  ...
  - アルファベット順： `m`: モード, `o`: 所有者, `g`: グループ, `n`: 名前, `k`: 種類, `l`: リンク名, `e`: エラー内容, `w`: 曜日, `u`: ユニーク文字列, `x`: 拡張子, `j`: 分類わけ, `5`: MD5（`make md5` ビルド時のみ）, `6`: git ステータス（`make git` ビルド時のみ）
  - サイズ順： `i`: inode番号, `h`: ハードリンク数, `s`: サイズ, `c`: カウント
  - 日付順： `d`: 日付, `t`: 時刻
  ---<br>
  - ソート条件に含まれない（`p`: ≒ `n`: 名前として扱われます, `[, ], |, ','` など: 無視されます）
- `-S`: ソートなし
- **優先順位**： `-S > -F`
<br><br>
