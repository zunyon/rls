# Version 0.5.0 current

## 使用方法
`rls [オプション]... [ディレクトリ名|ファイル名]...`

指定されたディレクトリやファイルを一覧表示します。何も指定されていない場合は，カレントディレクトリが一覧対象になります。
- **ロング形式**　：`/bin` はファイル，`/bin/` はディレクトリとして扱います
- **ショート形式**：`/bin`, `/bin/` 共にディレクトリとして扱います

オプションは個別に `-l -a -u` などと指定できますし，まとめて `-alu` のように指定しても構いません（順不同）。<br>
また，オプションと，ディレクト名，ファイル名も順不同です。<br>
下線付きの <ins>`-xxx`</ins> オプションは他のオプションとは別に指定します。

同一オプションが複数回指定された場合，後のオプションの内容に上書きされます。
- `-c`, `-p`, `-P`, `-f`, `-R`, `-F`, `-J`, `-n`

オプションには優先順位があります。（各オプションの最終行）<br>
優先順位の高いオプションが設定されている場合，優先順位の低いオプションは無視されます。
<br><br>

## 表示形式オプション
- `-s`：ショート形式（ファイル種別なし，1 色表示），デフォルトのショート形式より簡易表示
- `-l`：ロング形式
- `-j`：JSON 形式出力
- <ins>`-f`</ins>：ロング式の表示項目順を変更
  - デフォルト：`-fmogcdNKLE`
  - `m`: モード, `o`: 所有者, `g`: グループ, `c`: カウント, `d`: 日付, `p`: パス, `n`: 名前, `k`: 種類, `l`: リンク名, `e`: エラー内容
  - `i`: inode番号, `h`: ハードリンク数, `s`: サイズ, `t`: 時刻, `w`: 曜日, `u`: ユニーク文字列, `x`: 拡張子, `j`: 分類分け
  - `5`: MD5メッセージダイジェスト（`make md5` ビルド時のみ使用可能）
  - `6`: git ステータス（`make git` ビルド時のみ使用可能）
  - `[, ], |, ','`: 指定した文字が表示されます
  ---<br>
  - `d`: "%b %e %H:%M" もしくは "%b %e  %Y" フォーマット
  - `s`: ディレクトリとファイルのサイズ
  - `c`: 対象がファイルならファイルのサイズ，それ以外はディレクトリの中のエントリ数 (. と .. は除く)
  - `x`: 最後のピリオド以降の文字列（ピリオドがファイルの 1 文字目ではない条件）
  - `S, C, I`: カンマを含まない表示
  - `D, W`: 月，曜日などを省略せずに表示（英語表記）
  - `T`: 人間が読みやすい時刻形式で表示します
  - 大文字の場合はパディングなし（項目が固定長なら変化なし (m, t, 5, 6)）
- <ins>`-J`</ins>： `-fj` 向けファイルなどの分類分け，特定ファイルのラベル表示
  - 異なる種類のファイルを同じ分類に分ける（png, jpg, gif や、mov, avi, mp4 などを 1 分類に分ける）
  - 例えば `xImage=png,jpg,gif:xMovie=mp4,mov,avi:xAudio=mp3,wav` と指定した場合
    - `-f` の拡張子 `x` から `png` もしくは `jpg` もしくは `gif` が該当した場合，`Image`と表示する
    - `-f` の拡張子 `x` から `mp4` もしくは `mov` もしくは `avi` が該当した場合，`Movie`と表示する
    - `-f` の拡張子 `x` から `mp3` もしくは `wav` が該当した場合，`Audio`と表示する
  - 1 ファイルを完全に特定したい場合は，`i, I` で inode番号を指定してください（`INo Delete=123456789` など）
- **優先順位**： `-s > -j > -l = -f = -J > デフォルトのショート形式（ファイル種別あり）`
<br><br>

## ソートオプション
- <ins>`-F`</ins>：ソート順を変更
  - デフォルト：`-Fn`
  - 第 1，第 2，第 3，... のように複数のソート条件を指定
  - 同じアイテムを指定した場合は逆順ソート（`-Fnss` 第 1 ソート条件は名前，第 2 ソート条件はサイズ（逆順））
  - アルファベット順： `m`: モード, `o`: 所有者, `g`: グループ, `p`: パス, `n`: 名前, `k`: 種類, `l`: リンク名, `e`: エラー内容, `w`: 曜日, `u`: ユニーク文字列, `x`: 拡張子, `j`: 分類わけ, `5`: MD5（`make md5` ビルド時のみ）, `6`: git ステータス（`make git` ビルド時のみ）
  - サイズ順： `i`: inode番号, `h`: ハードリンク数, `s`: サイズ, `c`: カウント
  - 日付順： `d`: 日付, `t`: 時刻
  ---<br>
  - ソート条件に含まれない（`[, ], |, ','` など: 無視されます）
- `-S`: ソートなし
- **優先順位**： `-S > -F`
<br><br>

## 色指定オプション
- `-n`: 色なし表記
  - `-nn`:   色なし表記，ユニーク文字列を `[`, `]` で囲う
  - `-nnX`:  色なし表記，ユニーク文字列を `X`, `X` で囲う
  - `-nnXY`: 色なし表記，ユニーク文字列を `X`, `Y` で囲う
- <ins>`-c`</ins>: カスタム色設定（例：`-cbase=37:normal=34:...`）
  - 項目名：
    - `base`  ：ユニークでない時の共通色
    - `normal`：通常ファイルの色
    - `dir`   ：ディレクトリの色
    - `fifo`  ：FIFO/pipe の色
    - `socket`：socket の色
    - `device`：block/character デバイスの色
    - `label` ：ラベル文字列の色
    - `error` ：エラー時の色
    - `paint` ：検索文字列の色
  - 設定できる色・他：
    - 8色：   Control Sequence Introducer（terminal の実装による）
    - 256色： 30xx は文字の色, 40xx は背景色
    - 8 色と，256 色は同時に指定できない
- **優先順位**： `-n > -c > RLS_COLORS 環境変数 > デフォルト色`
<br><br>

## 色付けアルゴリズムオプション
- `-u`: より長い単語のユニーク文字列判定
- `-b`: ファイル名の先頭一致判定
- <ins>`-p`</ins>: 一致文字列の色付け（`-p文字列`，大小区別なし）
- `-e`: elisp のような，拡張子だけが異なるファイル名の "グループ" ユニーク文字列判定
- **優先順位**： `-p > -u > -b = -e > デフォルト`
<br><br>

## 出力内容オプション
- `-a`: "." から始まるファイルも表示
- `-o`: `-a` と併用，ディレクトリのみ表示
- `-O`: ファイルのみ表示
- <ins>`-P`</ins>: 一致文字列のファイル/ディレクトリ抽出表示（`-P文字列`，大小区別あり）
- **優先順位**： `-P = -O > -o > -a`
<br><br>

## 追加オプション
- `-i`: `-l` と併用，サイズを読みやすく
- `-r`: 集計結果，拡張子の集計結果を表示
- `-8`: n:name, p:path を OSC 8 形式で出力（ロング形式，ショート形式，標準入力）
  - `-8`: デフォルトのアプリケーションが実行される
  - `-8appname`: appname のアプリケーションを指定
- <ins>`-R`</ins>: 指定数字の文字列長さを paint 色で表示（`-R数字`）
- **優先順位**： `-i = -r = -R = -8`
<br><br>

## その他オプション
- `--`: 標準入力から読み込む，ユニーク文字列も標準入力をもとに計算
- `-h`, <ins>`--help`</ins>: このメッセージを表示
- `-v`, <ins>`--version`</ins>: バージョン情報を表示
- <ins>`--color=auto|always|never`</ins>: エスケープシーケンスの出力制御（デフォルト: auto）
  - `auto`: リダイレクト時自動判定
  - `always`: 常にリダイレクト時にも出力
  - `never`: リダイレクト時出力しない
<br><br>

## デフォルトカラーについて
  RLS_COLORS 環境変数：`-c` と同じ書式，同じ制限（set -x RLS_COLORS）<br>
  各アイテム名：base, normal, dir, fifo, socket, device, label, error, paint<br>
  デフォルト色：`base=37:normal=34:dir=36:fifo=33:socket=35:device=33:error=31:paint=32:reset=0`
